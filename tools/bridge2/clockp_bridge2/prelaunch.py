...
from .studio import ensure_edit_mode, play


def launch(session: Session, workspace: Path, prelaunch_path: Path, data: dict | None = None) -> dict:
    try:
        resolved_prelaunch_path = _resolve_workspace_path(workspace, prelaunch_path, "prelaunch_invalid_path", "prelaunch path must stay inside workspace")
...
def load_prelaunch_config(path: Path) -> dict:
    ...
    for index, item in enumerate(raw_steps):
        steps.append(_normalize_step(item, index))
    return {"steps": steps}
...
def execute_step(session: Session, workspace: Path, step: dict) -> dict:
    ...
    if kind == "code_sync_apply":
        ...
        project_path = _resolve_workspace_path(
            workspace,
            Path(step["project"]),
            "prelaunch_invalid_project_path",
            "code_sync_apply project path must stay inside workspace",
            {"step": step},
        )
        result = apply_code_sync(session, workspace, config_path, project_path, ensure_edit=bool(step["ensure_edit"]))
        return {
            "kind": kind,
            "name": step.get("name"),
            "config": step["config"],
            "project": step["project"],
            "ensure_edit": bool(step["ensure_edit"]),
            "result": result,
        }
...
def _normalize_step(value: object, index: int) -> dict:
    ...
    if kind == "code_sync_apply":
        config = value.get("config", "code-sync.roots.json")
        project = value.get("project", "default.project.json")
        ensure_edit = value.get("ensure_edit", True)
        if not isinstance(config, str) or not config:
            raise BridgeError("prelaunch_invalid_config", "code_sync_apply config must be a non-empty string", {"step_index": index, "step": value})
        if not isinstance(project, str) or not project:
            raise BridgeError("prelaunch_invalid_config", "code_sync_apply project must be a non-empty string", {"step_index": index, "step": value})
        ...
        return {"kind": kind, "name": name, "config": config, "project": project, "ensure_edit": ensure_edit}
...
def _run_shell_step(workspace: Path, step: dict) -> dict:
    cwd_value = step["cwd"]
    cwd = _resolve_workspace_path(workspace, Path(cwd_value), "prelaunch_shell_invalid_cwd", "shell step cwd must stay inside workspace", {"step": step})
