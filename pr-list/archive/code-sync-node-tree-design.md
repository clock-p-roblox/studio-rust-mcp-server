...
归档说明：这份设计记录保留历史讨论脉络，不再作为当前实现判定依据。最新 code-sync 事实请看 `roblox-dev-infra` 的 bridge 文档与 `go-helper` / `plugin-mcp2` 源码。

## 目标

把 code-sync 配置从旧的 `roots` 列表改成按 Studio DataModel 形状书写的 `tree`。bridge 和 task-agent 负责解释配置、扫描本地文件、推导文件 kind、组合 node/subnode，并把最终目标树发给 helper2/mcp2。helper2/mcp2 不理解 ownership / subnode 持有关系，只负责查询 Studio 子树和把给定目标树增量写入 Studio。
...
- Python bridge 与 Go task-agent 必须用同一组 golden fixture 验证 canonical Studio path、target hash 和 combined hash。
...
task-agent 必须和 bridge 解析同一份 `code-sync.tree.json`，并从同一棵最终 apply target 列表写入 `.clock-p/session.json` 的 code-sync binding。helper2 的 binding gate 继续校验 mapping profile 和 target `studio_path`；它不解析 tree 配置。
...
- `tools/bridge/clockp_bridge/code_sync/config.py`
  - 从 `{ "roots": [...] }` 改为 `{ "tree": {...} }`。
  - 输出 node 列表与 node/subnode 关系。
- `tools/bridge/clockp_bridge/code_sync/manifest.py`
  - 基于最终 apply targets 构建本地 manifest。
- `tools/bridge/clockp_bridge/code_sync/apply.py`
  - 下发最终 apply targets。
- `tools/bridge/clockp_bridge/code_sync/mapping.py`
  - 支持 node `$kind`、node-level `init.*` 规则和 subnode graft。
- `tools/bridge/test_code_sync.py`
...
- `go-helper/internal/taskagent/code_sync.go`
  ...
  - 使用和 bridge 一致的 canonical Studio path、配置 hash、target authority hash 规则。
...
- 相关 Go 测试
  ...
  - 共享 fixture 放在 repo 内固定 JSON 文件，例如 `tools/bridge/testdata/code_sync_tree_fixture.json`，Python 和 Go 测试必须读取同一份；fixture 至少覆盖 subnode graft、重复 local_path、显式 `$kind`、非 ASCII / 特殊符号 Studio segment。
...
### Phase 2：bridge manifest/apply 与 task-agent binding

- bridge 本地 manifest 基于最终 apply targets。
...
- 保持最终 live hash 验证。
- 构建 plugin，跑 bridge Python 测试与 go-helper 测试。
