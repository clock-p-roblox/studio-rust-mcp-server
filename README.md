...
- `launch` 读取 workspace 下的 `prelaunch.json`，按顺序执行 prelaunch steps，全部成功后再调用 `play`。
- `play` 是 direct Studio start 原语，不执行 build、flush 或其他 prelaunch workflow。
...
其中 `code_sync_apply` 默认 `ensure_edit = true`；如需保留“当前不在稳定 edit 就直接失败”的 direct 语义，必须显式写 `ensure_edit: false`。
